...
async def get_competitions(response: Response, db: AsyncSession = Depends(get_db)):
   try:
      ...
      if not competitions:
         return JSONResponse(
            format_error(status_code=status.HTTP_404_NOT_FOUND, error="competitions not found"),
            status_code=status.HTTP_404_NOT_FOUND)
         
      return {"competitions": competitions}
   except Exception as e:
         return JSONResponse(
            format_error(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, error=str(e)),
...
async def get_ranking(response: Response, name: str, db: AsyncSession = Depends(get_db)):
   try:
      obj = await db.execute(select(Competition).where(Competition.name==name.lower()))
...
async def create_competition(response: Response, body: CreateCompetitionSchema, db: AsyncSession = Depends(get_db)):
   try:
      ...
      return JSONResponse(
         format_error(status_code=status.HTTP_409_CONFLICT, error="only accepts 'meters' or 'seconds'"),
         status_code=status.HTTP_409_CONFLICT)
   except IntegrityError as e:
      return JSONResponse(
         format_error(status_code=status.HTTP_409_CONFLICT, error="name must be unique"),
         status_code=status.HTTP_409_CONFLICT)
   except Exception as e:
      return JSONResponse(
         format_error(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, error=str(e)),
